 ...
 */
public class ShopBagSumAllDetailsByXpath extends WebDriverTestBase{

    public Double valuealldiscount;
    public Double valueindividualunit;
    public Double valuesummedunits;
    public ArrayList<Double> up  = new ArrayList<>(); // Item Unit Price
    public ArrayList<Double> tp  = new ArrayList<>(); // Item Total Price
    public ArrayList<Double> qt  = new ArrayList<>(); // Item Quantity
    public ArrayList<Double> dc  = new ArrayList<>(); // ?
    public ArrayList<String> sk  = new ArrayList<>(); // Item Sku
    ...
    public ShopBagSumAllDetailsByXpath(List priceLinks, List qtyLinks, List skuLinks, String currency) {
        ...
        while (itrPriceLinks.hasNext()) {
            ...
            if (iprice != null && !iprice.trim().isEmpty()) {
                ...
                if (currency == "EUR") {
                    ...
                    pnowNumber = pnowNumber.replace(",", "");               // For prices over 1k we don't need it
                }
                ...
                if ((x % 2) == 0) {               // read is even so this is unit prices
                    ...
                    if (d == (0)) {               // if the unit price is £0 then its a free gift.
                        freebie = true;
        ...
        for (int i = 0; i < qt.size(); i++) {                               // && i < tp.size()
